...
(MS: yes, these are Abstract Data Types such as Lists and Sets but more on this later because this is an entirely different subject)
...
Let's illustrate this very simple concept by defining an account type **InterestAccount** which is an account with additional property `interestRate`. Here we can define this account as follows
...
`InterestAccount` shares three properties with already defined data model `Account`. This data model can be redefined using inheritance mechanism to inherit these 3 properties from `Account` data model . 
